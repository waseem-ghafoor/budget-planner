<template>
  <div class="container" id="printSection" v-if="loadedCompletely">
    <div class="row">
    ...
    <img src="/assets/images/logo.svg" width="500" height="65" class="float-left" v-if="PrintingMode">

    <div class="row">
      <div class="col-lg-6 col-md-6 col-sm-12">
        <p class="h5 float-left" v-if="view == 'planned'">
          Plan your Savings & Expenses <b-link id="tooltip-target-plan" style="text-decoration: none;" v-if="!PrintingMode">&#128161;</b-link>
          <b-tooltip target="tooltip-target-plan" triggers="hover">
            Here you can plan and visualize your future expenses.
          </b-tooltip>
          <a @click="printSection" v-if="!PrintingMode">🖨️</a>
        </p>
        <p class="h5 float-left" v-if="view == 'actual'">
          Track your Expenses <b-link id="tooltip-target-expense" style="text-decoration: none;" v-if="!PrintingMode">&#128161;</b-link>
        ...
        <p class="h5 float-left" v-if="view == 'monthly'">
          Monthly Summary <b-link id="tooltip-target-monthly" style="text-decoration: none;" v-if="!PrintingMode">&#128161;</b-link>
      ...
      <div class="col-lg-3 col-md-4 col-sm-12 float-right" v-if="!PrintingMode">
        <b-form-select v-model="view" :options="viewOptions"></b-form-select>
      </div>
    </div>
    <hr>

    <div class="row" :class="{'shadow-lg': !PrintingMode }"><div class="col">
        ...
        <table class="table-sm table-striped table-hover table-responsive sectioned" style="height: 75vh; overflow: auto;">
          ...
          <tbody v-for="(subCategories, category) in categories" :key="category" class="tbody-striped" :class="category.toLowerCase()">
            ...
            <tr v-for="(subCategory, idx) in subCategories" :key="subCategory.id">
              ...
              <td v-if="view == 'actual'" v-for="month in monthYear" :key="month[0]" >
                <div>
    ...
    <div class="right-bottom-fixed" v-if="view == 'planned'">
      <b-button class="add-recurring-plan" @click="showRecurringPlanModal = !showRecurringPlanModal" v-if="!PrintingMode">+</b-button>
    ...
    <div class="right-bottom-fixed" v-if="view == 'actual'">
      <b-button class="add-expense" @click="showAddExpenseModal = !showAddExpenseModal" v-if="!PrintingMode">+</b-button>
    ...
    <div class="right-bottom-fixed-2" v-if="view == 'actual'">
      <b-button class="view-logs" @click="viewLogsModal = !viewLogsModal" v-if="!PrintingMode">&#9737;</b-button>
...
export default {
  ...
  methods: {
    ...
    printSection() {
      ...
      setTimeout(() => { this.$htmlToPaper("printSection"); this.PrintingMode = false; }, 100);
    },
  ...
  mounted: function () {
    ...
    if (this.selectedYear !== null) {
      ...
      this.percentageSplitMonth = [];
      this.updateDoughnutGraph();
    }
    this.setUserProfile();
  },
  computed: {
    loadedCompletely: function() {
      return this.loadedCompletely1 && this.loadedCompletely2 && this.loadedCompletely3;
    },
    startingMonth: function() {
      return this.userProfile['starting_month'] + 1;
    },
    monthYear: function() {
      this.loadedCompletely5 = false;
      if(Number.isNaN(this.startingMonth)) { return []; }

      var tmp = [];
      var tmpYear = this.selectedYear;
      var tmpMonth = this.startingMonth;

      for (var i = 12; i > 0; i--) {
        tmp.push([tmpMonth, tmpYear]);
  ...
  data: function() {
    return {
      ...
      loadedCompletely1: false,
      loadedCompletely2: false,
      loadedCompletely3: false,
      loadedCompletely4: false,
      loadedCompletely5: false,
      PrintingMode: false,
